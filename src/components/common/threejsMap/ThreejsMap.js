...
class ThreejsMap {
    constructor(set) {
        ...
        this.divpos = this.getElementPos(this.set.container);
    ...
    vector3ToPosition(vector3) {
        const [left, top] = this.vector3ToPositionUtil(vector3, this.camera, this.width, this.height);
        return { left, top };
    }
     ...
     */
    vector3ToPositionUtil(vector3, camera, width, height) {
    ...
    lnglatToPosition(lnglat) {
        const vector3 = this.lnglatToVector32([...lnglat], '3d', this.set.center);
     ...
     */
    drawLine(vector3s) {
        const { color, linewidth } = this.set.lineStyle;
        const mater = new THREE.LineBasicMaterial({ color, linewidth });
     ...
     */
    drawMap() {
        let { features } = util.decode(this.mapData);
        // 计算省会坐标
        features.forEach(d => {
            d.vertor3 = [];
            d.geometry.coordinates.forEach((coordinate, i) => {
                ...
                coordinate.forEach((p, j) => {
                    if (p[0] instanceof Array) {
                        d.vertor3[i][j] = [];
                        p.forEach(e => {
                            d.vertor3[i][j].push(this.lnglatToVector32(e, '2d', this.set.center));
                        });
                    } else {
                        d.vertor3[i].push(this.lnglatToVector32(p, '2d', this.set.center));
                    }
                });
            });
        });
        ...
        features.forEach(d => {
            ...
            g.data = d;
            g.areaVector3 = this.lnglatToVector32([...d.properties.cp], '3d', this.set.center);
    ...
    init() {
        ...
        this.camera = new THREE.PerspectiveCamera(this.set.fov || 2, this.width / this.height, 1, 1000);
        this.setCamera();
        this.setLight();
        this.setRender();
